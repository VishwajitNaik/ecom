...
//   // Phone number validat <div id="recaptcha-container"></div>ion
//   const handlePhoneChange = (e) => {
//     const value = e.target.value.replace(/\D/g, "");
//     if (value.length <= 10) setPhone(value);
//   };
...
export default function PhoneOtpLogin({ onSuccess, onClose }) {
  ...
  // Use ref to persist recaptcha verifier across re-renders
  ...
  const initializeRecaptcha = () => {
    ...
    try {
      ...
      toast.error('Failed to initialize security verification. Please refresh.');
    }
  };
  ...
  const sendOtp = async () => {
    ...
    if (!recaptchaVerifierRef.current) {
      ...
      if (!recaptchaVerifierRef.current) {
        toast.error('Security verification not ready. Please wait and try again.');
        return;
      }
    }
    ...
    try {
      ...
      // Use the recaptcha verifier from ref
      const recaptcha = recaptchaVerifierRef.current;

      // Explicitly verify reCAPTCHA before sending OTP
      console.log('Verifying reCAPTCHA...');
      const recaptchaToken = await recaptcha.verify();
  ...
  const handleResendOtp = async () => {
    ...
    // Wait a bit for recaptcha to re-initialize, then verify explicitly
    setTimeout(async () => {
      try {
        setLoading(true);
        ...
        // Explicitly verify reCAPTCHA before sending OTP
        console.log('Verifying reCAPTCHA for resend...');
        const recaptchaToken = await recaptcha.verify();
        console.log('reCAPTCHA token obtained for resend:', recaptchaToken ? 'SUCCESS' : 'FAILED');
  ...
  // Verify OTP (same as before, but added resetRecaptcha on success)
  const verifyOtp = async () => {
    if (!otp.trim() || otp.length !== 6) {
      toast.error('Please enter a valid 6-digit OTP');
      return;
    ...
    try {
      ...
      const res = await fetch('/api/users/login', {
        ...
        body: JSON.stringify({ 
          phone: user.phoneNumber, 
          uid: user.uid, 
          guestId: (typeof window !== 'undefined' ? localStorage.getItem('guestId') : null) 
        }),
      });
      ...
      if (res.ok && data.success) {
        ...
        // Clean up recaptcha on successful login
        if (recaptchaVerifierRef.current) {
          recaptchaVerifierRef.current.clear();
          recaptchaVerifierRef.current = null;
        }
      ...
      <div className="relative w-full max-w-md transform transition-all duration-300 scale-100">
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-2xl overflow-hidden border border-gray-200/50">
          ...
          <div className="p-6 transition-all duration-300">
            ...
            {/* reCAPTCHA Container - Keep it in DOM */}
