...
export default function ProductsPage() {
  ...
  useEffect(() => {
    if (!containerRef.current || hasAnimatedRef.current || allItems.length === 0) return;
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-gradient-to-br -mt-5 pt-5 from-slate-300 to-indigo-100">
      ...
      <main className="container mx-auto px-3 lg:px-4 py-6 lg:py-8 text-gray-800 bg-white/80">
        ...
        {/* Single Products Section - All items together */}
        {allItems.length > 0 && (
          <section className="mb-12 lg:mb-16">
            <h2 className="section-title flex items-center justify-center text-xl lg:text-3xl font-bold mb-6 lg:mb-8 text-gray-800">
              <Image
            ...
            <div className="relative">
              ...
              <div className="hidden lg:block relative">
                ...
                >
                  {allItems.map((item, index) => (
                    <div key={`${item.itemType}-${item._id}`} className="flex-shrink-0 product-card">
                      <Product 
                        product={item} 
                        index={index}
              ...
              <div className="lg:hidden grid grid-cols-2 gap-3">
                {allItems.map((item, index) => (
                  <div key={`${item.itemType}-${item._id}`} className="w-full product-card">
                    <Product 
                      product={item} 
                      index={index}
                      onBuyNow={handleBuyNow}
